...
# ---------------------------------------------------------------------------
# _invoke_engine (mocked)
# ---------------------------------------------------------------------------

class TestInvokeEngine:
    def test_invokes_engine_with_correct_args(self, executor):
        step = {"step": 2, "name": "ui"}
        preamble = "PREAMBLE\n"

        mock_adapter = MagicMock()
        mock_adapter.run.return_value = (0, '{"result": "ok"}', "")
        executor._adapter = mock_adapter

        executor._invoke_engine(step, preamble)

        prompt_used = mock_adapter.run.call_args[0][0]
        assert "PREAMBLE" in prompt_used
        assert "UI를 구현하세요" in prompt_used

    def test_saves_output_json(self, executor):
        mock_adapter = MagicMock()
        mock_adapter.run.return_value = (0, '{"ok": true}', "")
        executor._adapter = mock_adapter
    ...
    def test_timeout_is_1800(self, executor):
        import inspect
        sig = inspect.signature(ex.ClaudeAdapter.run)
